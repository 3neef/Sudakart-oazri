<!-- latest jquery-->
<script src="<?php echo e(asset('assets/js/jquery-3.3.1.min.js')); ?>"></script>

<!-- Bootstrap js-->
<script src="<?php echo e(asset('assets/js/bootstrap.bundle.min.js')); ?>"></script>

<!-- feather icon js-->
<script src="<?php echo e(asset('assets/js/icons/feather-icon/feather.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/js/icons/feather-icon/feather-icon.js')); ?>"></script>

<!-- Sidebar jquery-->
<script src="<?php echo e(asset('assets/js/sidebar-menu.js')); ?>"></script>

<!--chartist js-->
<script src="<?php echo e(asset('assets/js/chart/chartist/chartist.js')); ?>"></script>

<!--chartjs js-->
<script src="<?php echo e(asset('assets/js/chart/chartjs/chart.min.js')); ?>"></script>

<!-- lazyload js-->
<script src="<?php echo e(asset('assets/js/lazysizes.min.js')); ?>"></script>

<!--copycode js-->
<script src="<?php echo e(asset('assets/js/prism/prism.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/js/clipboard/clipboard.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/js/custom-card/custom-card.js')); ?>"></script>

<!--counter js-->
<script src="<?php echo e(asset('assets/js/counter/jquery.waypoints.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/js/counter/jquery.counterup.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/js/counter/counter-custom.js')); ?>"></script>

<!--peity chart js-->
<script src="<?php echo e(asset('assets/js/chart/peity-chart/peity.jquery.js')); ?>"></script>

<!-- Apex Chart Js -->
<script src="https://cdn.jsdelivr.net/npm/apexcharts"></script>

<!--sparkline chart js-->
<script src="<?php echo e(asset('assets/js/chart/sparkline/sparkline.js')); ?>"></script>

<!--Customizer admin-->
<script src="<?php echo e(asset('assets/js/admin-customizer.js')); ?>"></script>

<!--dashboard custom js-->
<script src="<?php echo e(asset('assets/js/dashboard/default.js')); ?>"></script>

<!--right sidebar js-->
<script src="<?php echo e(asset('assets/js/chat-menu.js')); ?>"></script>

<!--height equal js-->
<script src="<?php echo e(asset('assets/js/height-equal.js')); ?>"></script>

<!-- lazyload js-->
<script src="<?php echo e(asset('assets/js/lazysizes.min.js')); ?>"></script>

<!--script admin-->
<script src="<?php echo e(asset('assets/js/admin-script.js')); ?>"></script>

    <!-- Datatable js-->
    <script src="<?php echo e(asset('assets/js/datatables/jquery.dataTables.min.js')); ?>"></script>
    <script src="<?php echo e(asset('assets/js/datatables/custom-basic.js')); ?>"></script>
    <!--ck editor-->
    <script src="<?php echo e(asset('assets/js/editor/ckeditor/ckeditor.js')); ?>"></script>
    <script src="<?php echo e(asset('assets/js/editor/ckeditor/ckeditor.custom.js')); ?>"></script><?php /**PATH C:\Users\10\Desktop\mazin projects\sudakart-latest-2023\resources\views/partials/scripts.blade.php ENDPATH**/ ?>